...
void callback(const moveit_msgs::msg::CartesianTrajectory& msg){
    inter = formation_mpc::utils::Interpolator::from_msg(msg);
    start = true;
    std::cout << "Start" << std::endl;
    Eigen::Vector6d vv;
    Eigen::Affine3d pp;
    for(double tt = 0; tt < inter.get_plan().time.back().seconds(); tt+=0.1)
    {
      std::cout << tt << std::endl;
      inter.interpolate(rclcpp::Time(static_cast<uint64_t>(tt * 1e9)), vv, pp);
